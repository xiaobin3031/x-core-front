...
export default function Ftp() {
    ...
    const handleDrop = async (e, dropIndex) => {
        ...
        await ajax.post('/ftp/sortFiles', updatedFiles.map((a, index) => ({
            id: a.id,
            fileFlag: a.fileFlag,
            sort: index
        })))
    ...
    return (
        <>
            <div className='ftp' onClick={closeAll}>
                <div className='ftp-head'>
                    <div className="btns">
                        ...
                        <span onClick={() => {
                            modalFlags.fileAdd = true;
                            setModalFlags({...modalFlags})
                        }}><FileAddIcon fill='green'/></span>
                        <span onClick={() => setAddFoldFlag(true)} className="fold-add">
              {!addFoldFlag && <FoldAddIcon fill='green'/>}
                            {!!addFoldFlag &&
                                <div>
                                    <Input onKeyDown={(e) => onEnter(e, foldNameChange)} label="文件夹名称"
                                           autoFocus={true}
                                           key="fold-add-key"/>
                                </div>
                            }
                            ...
                            return (
                                <div
                                    ...
                                    <div className='info'>
                                        <div className='name'>
                                            <label>
                                                <input name="file-checkbox" type="checkbox"
                                                       onClick={e => fileCheck(e, file)}/>
            ...
            {!!modalFlags.moveBatch &&
                <MoveDirModal sfiles={selectFiles.current} onOk={moveDirOk} onClose={moveDirClose}/>}
            {!!modalFlags.fileAdd && <AddFileModal onOk={addFileOk} onClose={addFileClose}/>}
            {!!modalFlags.renameAll &&
                <RenameAllModal sfiles={selectFiles.current} onOk={renameAllOk} onClose={renameAllClose}/>}
        </>
    )
}